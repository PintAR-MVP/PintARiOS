...
class ColorDetection: DetectionTask {

    typealias RGBA = (Double, Double, Double, Double)

    var result = CurrentValueSubject<RGBA, Never>((0, 0, 0, 0))
    ...
    static func convert(value: Any?) -> CurrentValueSubject<RGBA, Never>? {
        return value as? CurrentValueSubject<RGBA, Never>
    }
}
